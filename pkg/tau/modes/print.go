// print.go — stdout/stderr formatted output run-mode handler.
//
// RunPrint consumes the wired session's event stream and writes a
// human-readable transcript of one agentic turn to the configured
// stdout/stderr writers. The cli layer is in charge of argv parsing,
// Settings loading, and metadata subcommands (--help, --version); this
// handler only drives the session and formats what it emits.
package modes
...
func RunPrint(ctx context.Context, opts PrintOptions, session *tau.AgentSession) (*PrintResult, error) {
	if session == nil {
...
func buildPrintResult(session *tau.AgentSession, coll *eventCollector) (*PrintResult, error) {
	rt := session.Runtime()
	...
	if result.SessionID == "" {
		if hdr, ok := tree.Root().Payload.(tau.SessionHeaderPayload); ok {
			result.SessionID = hdr.SessionID
		}
	}
	for _, node := range path {
		if node.Kind != tau.KindMessage {
			continue
		}
		mp, ok := node.Payload.(tau.MessagePayload)
		if !ok {
			continue
		}
	...
	for i := len(result.Messages) - 1; i >= 0; i-- {
		if result.Messages[i].Role == string(tau.RoleAssistant) {
...
func extractText(blocks []tau.ContentBlock) string {
	out := ""
...
// handle type-switches on evt and updates the collector's tau.
func (c *eventCollector) handle(evt tau.SessionEvent) {
	switch e := evt.(type) {
	...
	case tau.MessageUpdateEvent:
		...
		if u, ok := e.Delta.(tau.UsageDelta); ok {
