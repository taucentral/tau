...
func RunRPC(ctx context.Context, opts RPCOptions, session *tau.AgentSession) error {
	if session == nil {
...
func newRPCServer(session *tau.AgentSession, out io.Writer) *rpcServer {
...
// forwardEvent converts a tau.SessionEvent to the matching notification
// and writes it. Unknown events are dropped.
func (s *rpcServer) forwardEvent(evt tau.SessionEvent) {
	switch e := evt.(type) {
