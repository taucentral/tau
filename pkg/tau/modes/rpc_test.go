...
// rpcTestHarness owns a real *tau.AgentSession plus an input buffer
...
func newRPCTestHarness(t *testing.T) *rpcTestHarness {
	t.Helper()
	client := tau.NewFauxProvider("rpc response")
...
func newSessionWithClient(t *testing.T, client tau.LLMClient) *tau.AgentSession {
	t.Helper()
	opts := tau.SessionOptions{
		Model:         "faux",
		Settings:      tau.DefaultSettings(),
...
func (b *blockingClient) Stream(ctx context.Context, _ tau.Request) (<-chan tau.Delta, error) {
	...
	go func() {
		...
		case <-ctx.Done():
			return
		case ch <- tau.TextDelta{Text: "released"}:
		}
		select {
		case <-ctx.Done():
			return
